...
function welcomeText(){
		...
		registerSounds(); // sound register function
...
function setBtn(e){
		...
		if(e.target.name=="playbutton"){
				...
				rightBlankImage.y=stageHeight/2-60;
...
function showCombination(){
	...
	leftText.x=140;
	leftText.y=stageHeight/2-20;
	gameContainer.addChild(leftText);
	
	if(setTutorial==false){
		rightText= new createjs.Text(rightWord,"65px poppinMd",colorNumber[randomColor]);
	}else{
		rightText= new createjs.Text(rightWord,"65px poppinMd",helpRandomNumber[0]);
	}

	rightText.x=stageWidth/2+70;
	rightText.y=stageHeight/2-20;
